...
import type { Member, BanModalState, MemberReviewsModalState } from '../types/admin.types';
    ...
    banMutation.mutate(
      { userId: banModal.member.id, reason },
      {
        onSuccess: () => {
          ...
          addToast(`${banModal.member!.name} has been banned`, 'success');
        },
  ...
  return (
    <div>
      <div className="glass-md rounded-xl overflow-hidden">
        ...
        {/* Table */}
        {isLoading ? (
          <div className="text-center py-12 text-white/40 text-sm">Loading members…</div>
        ) : isError ? (
          <div className="text-center py-12 text-red-400 text-sm">Failed to load members.</div>
        ) : members.length === 0 ? (
          <div className="text-center py-12 text-white/40 text-sm">No members found.</div>
        ) : (
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-white/8">
                <th className="text-left px-4 py-3 text-white/40 font-medium">Member</th>
                ...
                <tr key={member.id} className="hover:bg-white/[0.03] transition-colors">
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-3">
                      ...
                      {member.banned && (
                        <span className="shrink-0 text-xs font-medium px-2 py-0.5 rounded-full bg-red-500/15 text-red-400 border border-red-500/25">
                          Banned
                        </span>
                      )}
                    </div>
                  ...
                  <td className="px-4 py-3 text-right">
                    {member.banned ? (
                      <Button
                        ...
                        disabled={unbanMutation.isPending && unbanMutation.variables === member.id}
                      >
      ...
      <BanModal
        ...
        onClose={() => setBanModal({ isOpen: false, member: null })}
        onConfirm={handleConfirmBan}
        isSubmitting={banMutation.isPending}
        memberName={banModal.member?.name ?? ''}
      />
      <MemberReviewsModal
        isOpen={reviewsModal.isOpen}
        onClose={() => setReviewsModal({ isOpen: false, member: null })}
        member={reviewsModal.member}
      />
    </div>
  );
}
